...
RSpec.describe Node, type: :model do
  describe "# children" do
    ...
    let(:children_ids) { [child_a, grandchild_aa, grandchild_ab, great_grandchild_aba].map{ |child| child.id } }
    ...
    it { expect(root.all_children.map{ |child| child.id }).to include(*children_ids) }
    it { expect(root.all_children.map{ |child| child.id }).not_to include(*other_children_ids) }
  end
end
