...
RSpec.describe BirdsController do
	describe "GET nodes" do
		let(:subject) { get :nodes, params: { node_ids: node_ids } }
	    ...
	    let!(:root_bird) { create(:bird, node_id: root.id) }                                    #        |-r-
		  ...
		  let(:bird_ids) { [root_bird, child_a_bird, grandchild_aa_bird, grandchild_ab_bird, great_grandchild_aba_bird].map(&:id) }
		  ...
		  let(:node_ids) {
		  	[
		  		...
		  		great_grandchild_aba_bird
		  	]
		  }

			it { expect(subject.status).to eq(200) }
			it { expect(JSON.load(subject.body).symbolize_keys[:birds]).to include(*bird_ids) }
		end
	end
end
