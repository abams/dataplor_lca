...
class NodeControllerTest < ActionDispatch::IntegrationTest
  ...
  test "returns 404 if node_a not found" do
    # binding.pry
    get "/nodes/common_ancestor?a=#{nodes(:top).id}"
    
    assert_response :not_found
  end

  test "returns 404 if node_b not found" do
    get "/nodes/common_ancestor?b=#{nodes(:top).id}"
    
    assert_response :not_found
  end

  test "returns 200 if two present nodes found" do 
    get "/nodes/common_ancestor?b=#{nodes(:top).id}&b=#{nodes(:top).id}"
  ...
  #   get "/nodes/common_ancestor?b=#{nodes(:top).id}&b="
